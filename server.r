...
server<-function(input,output,session){
  ...
  output$map<-renderPlot({
    ...
    shiny::validate(need(input$x <= input$y,"The Start Year cannot be after the End Year. Please adjust the time range.")) %then%
      shiny::validate(need(states() != "",""))
    Enrollment_Map(x=input$x,y=input$y,z=states(),countylevel=input$countylevel,borders=input$borders)})
  output$title<-renderText({
    shiny::validate(need(states() != "",""))
    ifelse(input$x == input$y,paste("ACEP-WRP Enrollment ",input$x,sep=""),
           paste("ACEP-WRP Enrollment ",input$x," to ",input$y,sep=""))
  })
}